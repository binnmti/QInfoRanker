...
2. **タスク実行**: `/plan-run` でサブエージェントを使って順次実行
3. **結果記録**: 各タスクファイルの `## 実行結果` セクションに自動記録
4. **振り返り**: `RETROSPECTIVE.md` に問題点・改善点を蓄積
5. **学びの昇格**: 重要な学びは `LESSONS_LEARNED.md` → `CLAUDE.md` へ反映

### サブエージェント使用の原則

- 各タスクは `Task` ツールでサブエージェントを起動して実行
- コンテキストを分離し、タスクの独立性を保つ
- 結果は必ずタスクファイルにフィードバックとして記録
