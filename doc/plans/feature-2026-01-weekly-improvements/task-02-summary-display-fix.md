...
要約が表示されない原因を診断・通知する機能を追加。`SummaryDiagnostics` レコードで AI設定状態、カテゴリ別記事数、生成可否、ブロック理由を一元管理。UIに診断情報を表示し、生成できない場合は具体的な理由（記事不足、AI未設定など）をユーザーに明示するようにした。
...
- [x] 要約生成の前提条件（記事数、IsRelevant、LlmScore）の診断情報を表示
- [x] 要約生成失敗時のエラーメッセージをUIに表示（現状はログのみ）
- [x] Azure OpenAI 設定（WeeklySummary セクション）の確認と修正
- [x] 手動生成ボタンのエラーハンドリング改善

## テスト

- [x] 条件を満たさない場合に適切なメッセージが表示されることを確認
- [x] 条件を満たす場合に要約が生成・表示されることを確認
- [x] Azure OpenAI エラー時に適切なメッセージが表示されることを確認
- [x] 既存のビルドが通ることを確認
