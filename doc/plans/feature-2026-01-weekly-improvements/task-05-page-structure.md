...
ルートを `/weekly` と `/weekly/{keyword}` に変更し、キーワードごとにユニークなURLでアクセス可能に。コンボボックスをボタングループに変更してURL連動させ、キーワード一覧ページにも「おすすめ」ボタンを追加。日本語キーワードは `Uri.EscapeDataString` でエンコード対応した。
...
- [x] ページのルートを `/weekly/{keyword?}` に変更（キーワードはオプショナル）
- [x] キーワード未指定時はキーワード一覧または最初のキーワードを表示
- [x] ナビゲーションメニューの表示名を「今週のおすすめ」に変更
- [x] キーワード選択時にURLが変わるように（コンボボックス廃止 or URL連動）
- [x] キーワード一覧からのリンクを各キーワードのURLに変更

## テスト

- [x] `/weekly/量子コンピューター` でアクセスできることを確認
- [x] `/weekly/IT` でアクセスできることを確認
- [x] `/weekly` でデフォルト表示されることを確認
