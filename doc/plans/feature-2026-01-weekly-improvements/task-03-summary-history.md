...
- [ ] WeeklySummary エンティティの確認（GeneratedAt で履歴管理可能か）
- [ ] 同一週に複数の要約を保持できるようにする（現状は KeywordId+WeekStart でユニーク）
- [ ] 履歴取得メソッドの追加（日付順で過去の要約を取得）
- [ ] UI に左右ボタンを追加（前の要約/次の要約）
- [ ] 現在表示中の要約の生成日時を表示
